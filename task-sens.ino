...
void test(JsonDocument &doc)
{
  JsonArray arr = doc["sens"];
  // sensorLoad(doc.to<JsonObject>());
  // trace("2.2."+String(doc.memoryUsage()));
  trace("2.3." + String(arr.size()));
...
void setup()
{
  mem = checkMemoryFree();
  RTC.begin();
  app.startTime = RTC.now().secondstime();
